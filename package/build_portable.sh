...
OUTPUT_TAR="openclaw-portable.tar.gz"
...
if [ -d "$HOME/.openclaw" ]; then
    ...
    # 4.1.1 仅在 $OPENCLAW_HOME 这一层做白名单过滤（避免把未知目录/文件带进绿色包）
    # 说明：这里只控制顶层目录/文件是否存在；更细粒度的‘清理内容’仍由后续规则完成。
    ALLOW_TOP_ITEMS=(
        'credentials'
        'extensions'
        'feishu'
    ...
    # 删除顶层非白名单条目
    find "$OPENCLAW_HOME" -mindepth 1 -maxdepth 1 -print0 | while IFS= read -r -d '' item; do
        base_name=$(basename "$item")
        keep=false
        for allowed in "${ALLOW_TOP_ITEMS[@]}"; do
    ...
    echo "   -> 已按白名单过滤 OPENCLAW_HOME 顶层目录/文件"
    ...
    if [ -d "$WORKSPACE_DIR" ]; then
        find "$WORKSPACE_DIR" -mindepth 1 -maxdepth 1 | while read -r item; do
            ...
            # 保留规则 A: 名字叫 skills 的目录（用户要求：不要过滤 skills 下面的技能）
    ...
    # 4.3 清理运行痕迹（保留目录骨架，保证部署后可直接可用）
    # 说明：agents/subagents/memory/canvas/completions/delivery-queue/devices/cron/.clawhub/identity 等已由顶层白名单剔除
    # 这里只对仍保留的目录做内容级瘦身（logs/media/extensions/workspace）

    # logs: 不保留任何历史日志（但保留目录骨架，保证运行时可写）
    if [ -d "$OPENCLAW_HOME/logs" ]; then
        rm -rf "$OPENCLAW_HOME/logs"
    fi
    mkdir -p "$OPENCLAW_HOME/logs"
    echo "   -> 已清空 logs（不保留任何日志文件）"

    # media: 清空 inbound 里的缓存文件（保留目录）

    if [ -d "$OPENCLAW_HOME/media" ]; then
        rm -rf "$OPENCLAW_HOME/media"
        mkdir -p "$OPENCLAW_HOME/media/inbound"
        echo "   -> 已清空 media（保留 inbound 目录骨架）"
    fi

    # 4.4 extensions: 全部保留（按需求不做精简）
    if [ -d "$OPENCLAW_HOME/extensions" ]; then
        echo "   -> 已保留 extensions（不做过滤）"
    fi
...
#!/bin/bash
# 核心修复：自动解析真实绝对路径，防止全局软链接调用时找不到内置环境
...
tar -czf "$OUTPUT_TAR" "$PORTABLE_DIR"

echo "=================================================="
echo "🎉 批量部署绿色工程打包完成！"
echo "📦 最终分发包: $OUTPUT_TAR"
echo "=================================================="
